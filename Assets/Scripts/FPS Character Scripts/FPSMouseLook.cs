...
public class FPSMouseLook : MonoBehaviour
{
    ...
    void HandleRotation() {
        if (currentSensivity_X != mouseSensivity || currentSensivity_Y != mouseSensivity) {
            currentSensivity_X = mouseSensivity;
            currentSensivity_Y = mouseSensivity;
        }

        xSensitivity = currentSensivity_X;
        ySensitivity = currentSensivity_Y;

        if (axes == RotationAxes.MouseX) {
            rotation_X += Input.GetAxis("Mouse X") * xSensitivity;
        ...
        if (axes == RotationAxes.MouseY) {
            rotation_Y += Input.GetAxis("Mouse Y") * ySensitivity;
